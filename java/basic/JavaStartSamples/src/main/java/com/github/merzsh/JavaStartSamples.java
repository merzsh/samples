...
public class JavaStartSamples {
	...
	*/
	...
	public static void numbersCharsStrings() {
		...
		num = 0b1010; System.out.print("Бинарный литерал '0b1010': " + num + "; ");
		num = 0xFF; System.out.println("Шестнадцатиричный литерал '0xFF': " + num);
		num = 123_456; System.out.print("Десятичный литерал '123_456', разделенный подчеркиванием: " + num);
		System.out.println();
	...
	public static void checkThreads() {
		...
		for(int i=0; i<lviLen; i++){
				lriList.add(Integer.toString(i));
		}
	...
	public static void checkSimpleSQL() {
		...
		String STR_DB_USERPASS = "javapass"; //"pass";
	...
	private static class DataProviderHibSpec extends DataProvider {
		private static final String MSG_SESSION_CAST_ERROR =
				"mriConnection member is not Session class instance, error!";
	...
	private static class DaoLayer implements IDaoLayer {
	    ...
	    try {
				...
				String lvsHQL =
						"SELECT NEW dummy.Dummy$SelectionView(u.id, u.name, c.id, c.model) " +
	...
	for(int i=1; i<=2; i++){
	    ...
	    for(SelectionView v : lv){
		System.out.println(v);
	    }

	    System.out.println("Query SelectionView fields with records filtered by User age " +
			       "(lower '20') ...");
	    ...
	    System.out.println("Query SelectionView fields with records filtered by Car color " +
			       "(equal 'Red') ...");
	...
	public String getUserName(){
	    if( mvsUserName == null ) mvsUserName = new String();
	    return mvsUserName;
	}
	...
	@Override
	public String toString() {
	    return String.format("User info: { UserID = '%d', UserName = '%s' }",
				 mviUserId, mvsUserName);
	}
    ...
    public static class Computer extends TopClass
	implements IComputer, MethodReplacer, ApplicationListener {
	...
	public boolean equals(Object otherObject){
	    ...
	    return Objects.equals(mvsVendor, obj.mvsVendor) &&
		Objects.equals(mvsSegment, obj.mvsSegment) &&
	...
	@Override
	public int evaluate(int pviArg1, int pviArg2, String pvsMathAction){
	    //System.out.println("I'm an evluate() method of Dept class!");
		...
		break;
	    default:
		String str = "Incorrect math action '%s' was passed in method %s. " +
		    "There are '+' and '*' supported only!";
	...
	public boolean equals(Object otherObject){
		...
		if(this == otherObject) return true;
		    ...
		    return mviDeptId == obj.getDeptId() &&
			Objects.equals(mvsDeptName, obj.getDeptName()) &&
	...
	public Set<Integer> getUsersId(){
	    ...
	    if(mriUsersId != null){
		for(Integer i : mriUsersId){
		    result.add( Integer.valueOf(i.intValue()) );
		}
	    }
	    return result;
	}
	...
	public Object logMethAround(ProceedingJoinPoint joinpoint,
				    int pviArg1, int pviArg2, String pvsMathAction) {
	    try{
		String str = "---> BEGIN method with args: int '%d', int '%d', String '%s'";
		str = String.format(str, pviArg1, pviArg2, pvsMathAction);
		System.out.println(str);
		Object result = joinpoint.proceed();
		System.out.print("--->"); logMethSimpleEnd();
	...
	try{
		...
		new ClassPathXmlApplicationContext("./src/dummy/dummy_spring.xml");
	    ...
	    System.out.println("Readed file property value: some.property=" +
			       dao.getSomeProperty() );
	...
	public CacheManager cache() {
		...
		jCacheManager.createCache(
				"my_cache",
				Eh107Configuration.fromEhcacheCacheConfiguration(orgEhcacheConfig)
		);
